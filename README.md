...
- **ワード検出**: デフォルトで `TODO` / `FIXME` をハイライト表示（`GIT_REVIEW_TOOL_KEYWORDS` 環境変数またはUIで変更可能）。差分内のマッチを黄色ハイライトし、左サイドバーの各ファイルにマッチ件数バッジを表示
- hunk ごとにコメントを入力・保存
- hunk コメントの削除
...
| `GIT_REVIEW_TOOL_KEYWORDS` | `TODO,FIXME` | ワード検出キーワード（カンマ区切り）。起動時にストレージへシードされ、UI からも追加・削除可能 |
